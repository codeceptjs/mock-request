...
module.exports = class PuppeteerCodeceptJSAdapter extends Adapter {
  static get name() {
    return 'puppeteer';
  }
  get defaultOptions() {
    ...
    this[LISTENERS].set(page, {
      request: async request => {
        if (requestResourceTypes.includes(request.resourceType())) {
          ...
          // filter out requests
          if (this.options.filter && !url.matches(this.options.filter)) {
            return request.continue();
          }

          const parsedUrl = new URL(url);

          if (this.options.host && parsedUrl.host !== this.options.host) {
            return request.continue();
          }

          if (this.options.path && !parsedUrl.pathname.startsWith(this.options.path)) {
            return request.continue();
          }          
      ...
      requestfinished: request => {
        ...
        if (passthroughs.has(request)) {
          if (!passthroughs.get(request)) return;
  ...
  async passthroughRequest(pollyRequest) {
    ...
    if (parsedUrl.query[PASSTHROUGH_REQ_ID_QP]) {
      return; // don't pass through two times
    }
